...
class _RecordVisitPageState extends ConsumerState<RecordVisitPage> {
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isRecording || _isLocating || !_isWithinRange
                    ? null
                    : _recordVisit,
  ...
  Future<void> _recordVisit() async {
    ...
    final request = RecordVisitRequest(
    ...
    if (mounted) {
      ...
      if (response.success) {
        ...
        context.go('/field/visits');
      } else if (response.error?.code == 'NETWORK_ERROR') {
        await ref
            .read(offlineQueueRepositoryProvider)
            .enqueue(
              OfflineQueueItem(
                id: 'visits.record-${DateTime.now().microsecondsSinceEpoch}',
                feature: 'visits',
                operation: 'visits.record',
                payload: {
                  ...request.toJson(),
                  'isOffline': true,
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم حفظ الزيارة للمزامنة عند عودة الاتصال'),
          ),
        );
        context.go('/field/visits');
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response.error?.message ?? 'فشل تسجيل الزيارة'),
          ),
        );
      }
    }
